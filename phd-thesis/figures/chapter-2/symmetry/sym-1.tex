...
\node[anchor=south,inner sep=0mm](s1) at (current page.south)
	{
	\includegraphics[width=\textwidth,trim={0cm 1cm 0 0},clip]{/media/labfiles/ruco/phd-ssp/phd-codes/symmetry/symmetry5-algaas}
	};
...
\shade [ball color=As,white] ([xshift=5mm,yshift=3.0cm]s1.west) circle (0.2) node[label=right:{\Large\, As}] { };
...
\begin{scope}[xshift=2cm,yshift=-5.25cm,rotate around z=0]
	\draw[very thick,-{Stealth},blue] (0,0,0) -- (1,0,0) 
			node[anchor=west,white]{$z$};
	\draw[very thick,-{Stealth},red] (0,0,0) -- (0,1,0) 
			node[anchor=south,white]{$y$};
	\draw[very thick,-{Stealth},green] (0,0,0) -- (0,0,1) 
			node[anchor=north,white]{$x$};
\end{scope}
\end{tikzpicture}
