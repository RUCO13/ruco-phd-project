...
\pgfplotstableread[col sep=comma]{/media/labfiles/RUCO/PhD-PHOTONICS/PhD-CODES/DATA/M4_3140-mapabs001.csv}\map
\pgfplotstableread[col sep=comma]{/media/labfiles/RUCO/PhD-PHOTONICS/PhD-CODES/DATA/M4_3521-mapabs001.csv}\mapd
\pgfplotstableread[col sep=comma]{/media/labfiles/RUCO/PhD-PHOTONICS/PhD-CODES/DATA/M4_3523-mapabs001.csv}\mapt
...
\begin{document}
	\begin{tikzpicture}
		...
		\begin{groupplot}[group style={group size=1 by 3,vertical sep=15mm},height=3cm,width=10cm,
			...
			\draw[densely dashed,red]  (axis cs:300,600)--(axis cs:300,700);
			\draw[densely dashed,red]  (axis cs:350,600)--(axis cs:350,700);
				\node[anchor=east,white,font=\bf] at (axis cs: 580,695){a) M4\_3140};
			\nextgroupplot[	
			colorbar,
			every colorbar/.append style={ylabel=Absorption $\left(1/\mathrm{nm}\right)$,
				%ymin=0.00025,ymax=0.0016,
			...
			\addplot3[surf,shader=interp] table[x index=0,y index=1,z index=2] {\mapd};
			\draw[densely dashed,red]  (axis cs:300,600)--(axis cs:300,700);
			\draw[densely dashed,red]  (axis cs:350,600)--(axis cs:350,700);
				\node[anchor=east,white,font=\bf] at (axis cs: 580,695){b) M4\_3521};
			\nextgroupplot[
			colorbar,
			every colorbar/.append style={ylabel=Absorption $\left(1/\mathrm{nm}\right)$,
				%ymin=0.0001,ymax=0.0025,
				major tick length = 1.5mm,
				scaled y ticks=false,
				ytick align=inside,
				ytick pos=right,
			...
			\addplot3[surf,shader=interp] table[x index=0,y index=1,z index=2] {\mapt};
			\draw[densely dashed,red]  (axis cs:300,600)--(axis cs:300,700);
			\draw[densely dashed,red]  (axis cs:350,600)--(axis cs:350,700);
			
			
			\node[anchor=east,white,font=\bf] at (axis cs: 580,695){c) M4\_3523};
 		\end{groupplot}
	\end{tikzpicture}
\end{document}
