...
\pgfplotstableread[col sep=comma]{../../../../scripts/data/ras-m43521-3l4.dat}\rasuno
\pgfplotstableread[col sep=comma]{../../../../scripts/data/ras-m43521-l4.dat}\rasdos
...
\begin{axis}[
	...
	\addplot[color=Set1-A] table[x index=0,y expr=\thisrowno{1}*1.6]
	\rasuno;
	% \addlegendentry{(a) RAS $	\circlearrowright$};
	\addplot[color=Set1-B] table[x index=0,y expr=\thisrowno{1}*1.6]\rasdos;
	% \addlegendentry{(b) RAS $\circlearrowleft$};

	\node[scale=2,color=Set1-A] at (axis cs:1.54,4e-4){(a) RAS  $\circlearrowright$};
    \node[scale=2,color=Set1-B] at (axis cs:1.54,-3e-4){(b) RAS  $\circlearrowleft$};
