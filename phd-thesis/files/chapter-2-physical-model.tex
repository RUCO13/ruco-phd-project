...
\end{figure}               
The barrier width as can see plays an important role, because the width define tunneling of electrons and holes, for example \Cref{subfig:chapter-2-sec-numerical-results-b} and \Cref{subfig:chapter-2-sec-numerical-results-d}  structures has the same wells widths with exception to barrier type, the first one is \algaasx{0.15} and the width is $b_{w}=1.98$ while the second one is AlAs type with a width of $b_{w}=0.565$. It's knows that the difference of these is the band gap energy then, as is expected, the tunneling is less than the \algaasx{0.15}  barrier. With respect to tunneling, if compares the \Cref{subfig:chapter-2-sec-numerical-results-a} and \Cref{subfig:chapter-2-sec-numerical-results-c} structures, which basically are the same structures except by the width of second well, the wave function in one of this is symmetrically localized as doublets states, while the other the wave function practically is well localized in one of wells as single state energy of a single QW\cite{sivalertporn2016effectofbarrier}. 
