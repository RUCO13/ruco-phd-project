...
Therefore, this method simplifies the solution of any structure with arbitrary or in our case dependent on band-edge potential. The next task is over to define the potential profile and their dependence of  effective-mass and band gap energy.  Firstly, we take as reference the double quantum wells propose by Harrison et al.\cite{harrison2016chap3} and reproduce the  eigenenergies and wave functions as shows in \Cref{fig:chapter2-sec-numerical-calculations-harrison-wells}.For this structure, it takes into account symmetric double wells, that is to say, with same width $l_{w}=6$nm, \algaasx{0.2} central barrier of $l_{b}=6$.  
