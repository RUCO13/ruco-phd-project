% !TeX document-id = {0d4990b5-a774-498d-ad7b-f9108372dea8}
% !TeX TXS-program:pdflatex = pdflatex -synctex=1 -interaction=nonstopmode --shell-escape -output-directory=build %.tex
...
\newcommand{\qwplot}{%
	\begin{tikzpicture}[trim axis left,trim axis right]
		...
		\begin{axis}[scale only axis,
			...
			axis lines = middle,
			y axis line style={densely dashed,line width=0.4mm},
			...
			every axis x label/.style= {at={(current axis.right of origin)},anchor=south east,scale=1.25,
							xshift=20,
							yshift=-18},
			ytick=\empty,]
	...
	\pgfplotsinvokeforeach {7,20,33, 46,59,72,85} {
		\draw[line width=0.3mm,blue,{Stealth[scale=1.25]}-] (axis cs:-5, ##1) -- (axis cs:0, ##1);}
	
	\pgfplotsforeachungrouped \x/\y in {35/7,47/20,36/33, 38/46,35/59,35/72,38/85}{
		\edef\temp{\noexpand\draw[line width=0.3mm,blue,-{Stealth[scale=1.25]}] (axis cs:\x, \y) -- (axis cs:\x+5, \y);	}\temp
	}
		\end{axis}
	\end{tikzpicture}
}
...
\begin{document}
	...
	font=\fontsize{25pt}{1}\selectfont,
	...
	every last row/.style={after row=[2.25cm]\bottomrule\bottomrule},
	after row ={[1.4cm]},
	every row no 5/.append style={after row={[3mm]\multicolumn{2}{c}{ }\\}},
	columns/names/.style={column type=l,string type,column name=Sample name},
columns/plots/.style={
		column name={$V(z)$},
		assign cell content/.code={% use \multirow for Z column:
			\ifnum\pgfplotstablerow=0
			\pgfkeyssetvalue{/pgfplots/table/@cell content}
			{\multirow{\numberofrows}{4cm}{\qwplot}}%
			\else
			\pgfkeyssetvalue{/pgfplots/table/@cell content}{}%
			\fi
		}
	...
	columns/quantity/.style={column type=c,column name=$\left[\dfrac{1}{cm^{3}}\right]$,
